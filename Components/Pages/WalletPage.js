	...
	// const local = await JSON.parse(window.localStorage.getItem("member"));
		...
		addMoney.addEventListener('click',async (e) => {
			...
			const options = {
				...
				const responseAdd = await fetch("/https://vinced.azurewebsites.net/members/addCredits", options); // fetch return a promise => we wait for the response
		...
		removeMoney.addEventListener('click',async (e) => {
			...
			const options = {
				...
				const responseRemove = await fetch("/https://vinced.azurewebsites.net/members/removeCredits", options); // fetch return a promise => we wait for the response
...
async function addOnClick(Idmember) {
	...
	try {
		...
		const response = await fetch("/https://vinced.azurewebsites.net/users/addCredits", options); // fetch return a promise => we wait for the response
...
async function removeOnClick(member,actualBalance) {
		...
		try {
			...
			const response = await fetch("/https://vinced.azurewebsites.net/users/removeCredits", options); // fetch return a promise => we wait for the response
