...
async function connectedProductPage() {
  ...
  let response = await fetch(`https://vinced.azurewebsites.net/articles?id=${idProduct}`, request);
  response = await response.json();
  let isLikedResponse = await fetch(`https://vinced.azurewebsites.net/favorites?id_membre=${idMember}&id_article=${idProduct}`);
  isLikedResponse = await isLikedResponse.json();
  const product = response[0];
  
  const adresse = await fetch(`https://vinced.azurewebsites.net/adresses?id=${product.id_adresse}`);
...
`;
  ...
  if (buyButton) {
    buyButton.addEventListener('click', async (e) => {
      ...
      await fetch(`https://vinced.azurewebsites.net/articles/buy`, request);
      await renderhomepage();
    });
  }
}

async function disconnectedProductPage() {
